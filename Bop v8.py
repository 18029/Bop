...
# Setting the song speedn and the status of the song
play_music = False
song_timer = 0
song_speed = 4
...
score_bg = pygame.image.load("image/score_bg1.png").convert_alpha()
...
class Note_timing:
    ...
    def get_y(self):
        ...
        for i in range(len(self.timing)):
            ...
            self.song.append(-(song_speed * 60) * (self.timing[i]) + 485)
...
def arrow_spawn():
    ...
    for i in range(len(rarrowY)):
        rarrow_group.add(Arrow("rarrow", (525), rarrowY[i], 0, song_speed))
    for i in range(len(darrowY)):
        darrow_group.add(Arrow("darrow", (425), darrowY[i], 0, song_speed))
    for i in range(len(larrowY)):
        larrow_group.add(Arrow("larrow", (225), larrowY[i], 0, song_speed))
    for i in range(len(uarrowY)):
        uarrow_group.add(Arrow("uarrow", (325), uarrowY[i], 0, song_speed))



# Button
start_button = Img_Button(400, 300, start, click_start)
replay_button = Img_Button(550, 400, replay, replay)

unravel_button = Word_Button(300, 10, "Unravel")
rick_button = Word_Button(300, 110, "Rick")

resume_button = Word_Button(165, 450, "Resume")
quit_button = Word_Button(465, 450, "Quit")

tutorial_button = Word_Button(10, 525, "Tutorial")

back_button = Word_Button(10, 525, "Back")
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == arrow_timer and start_game == True:
            song_timer += 1
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # Quits to menu
            if event.key == pygame.K_ESCAPE and play_game == False and start_game == True:
                menu_screen = True

            if event.key == pygame.K_ESCAPE and play_game == True and start_game == True:
                a = iter(rarrow_group)
    ...
    # Shows menu
    elif start_game == True and menu_screen == True:
        ...
        music_load = False
        if unravel_button.draw_wbutton():
            if music_load == False:
        ...
        if rick_button.draw_wbutton():
            print("yo")

        if tutorial_button.draw_wbutton():
            tutorial_screen = True
            menu_screen = False


    elif tutorial_screen == True and menu_screen == False:
        screen.blit(tutorial, (0, 0))
        
        if back_button.draw_wbutton():
    ...
    elif start_game == True and play_game == False and paused_screen == True and menu_screen == False:
        screen.blit(start_bg, (0, 0))
        screen.blit(paused, paused_rect)
        pygame.mixer.music.pause()

        if resume_button.draw_wbutton():
            for i in (rarrow_group):
                i.unpause()
            for i in (darrow_group):
                i.unpause()
    ...
    elif start_game == True and play_game == False and end_screen == False and menu_screen == False:
        screen.blit(start_bg, (0, 0))
