...
class Note_timing:
    ...
    def get_y(self):
        ...
        for i in range(len(self.timing)):
            ...
            self.song.append(-(song_speed * 60) * (self.timing[i]) + 500)
...
class Arrow(pygame.sprite.Sprite):
    ...
    def hit(self):
        ...
        if rarrow_key == "pressed":
            if (self.rect.y <= 520 and self.rect.y >= 450):
                self.kill()
                healthbar.sprite.get_health(10)
                p_health += 10
                score_value += 1
            elif self.rect.y < 425 and self.rect.y >= 375:
                healthbar.sprite.get_damage(200)
                p_health -= 75
                self.kill()   
...
replay_button = Word_Button(450, 350, "Replay")
